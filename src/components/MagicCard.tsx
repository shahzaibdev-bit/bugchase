...
export const MagicCard = ({
  ...
  inverted = false,
  enableSpotlight = true // New prop to toggle spotlight
}: {
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      ...
      // Mouse enter tilt effect (subtle kick)
      if (enableTilt) {
        gsap.to(element, {
            rotateX: inverted ? -5 : 5, 
            rotateY: inverted ? -5 : 5,
            duration: 0.3, 
            ease: 'power2.out',
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (enableTilt) {
        const tiltMultiplier = inverted ? -1 : 1;
        const rotateX = ((y - centerY) / centerY) * -10 * tiltMultiplier;
        const rotateY = ((x - centerX) / centerX) * 10 * tiltMultiplier;
  ...
  }, [animateParticles, clearAllParticles, disableAnimations, enableTilt, enableMagnetism, clickEffect, glowColor, inverted]);

  // Spotlight Logic (derived from react-bits SpotlightCard)
  const divRef = useRef<HTMLDivElement>(null);
  const [position, setPosition] = useState({ x: 0, y: 0 });
  const [opacity, setOpacity] = useState(0);

  const handleSpotlightMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!enableSpotlight || !divRef.current) return;
